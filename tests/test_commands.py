...
from xdiabetes.providers.openai_codex_provider import _strip_model_prefix
from xdiabetes.providers.registry import find_by_model
...
def mock_agent_runtime(tmp_path):
    ...
    with patch("xdiabetes.config.loader.load_config", return_value=config) as mock_load_config, \
         ...
         patch("xdiabetes.x_diabetes.workspace.prepare_xdiabetes_workspace") as mock_prepare_workspace, \
...
def test_agent_config_sets_active_path(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        "xdiabetes.config.loader.set_config_path",
    ...
    monkeypatch.setattr("xdiabetes.x_diabetes.workspace.prepare_xdiabetes_workspace", lambda _path, mode, silent=False: None)
    ...
    monkeypatch.setattr("xdiabetes.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("xdiabetes.cli.commands._print_agent_response", lambda *_args, **_kwargs: None)
...
def test_gateway_uses_workspace_from_config_by_default(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("xdiabetes.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr(
        "xdiabetes.x_diabetes.workspace.prepare_xdiabetes_workspace",
...
def test_gateway_uses_config_directory_for_cron_store(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("xdiabetes.cron.service.CronService", _StopCron)
