...
import xdiabetes.channels.matrix as matrix_module
from xdiabetes.bus.events import OutboundMessage
from xdiabetes.bus.queue import MessageBus
from xdiabetes.channels.matrix import (
    MATRIX_HTML_FORMAT,
    TYPING_NOTICE_TIMEOUT_MS,
    MatrixChannel,
)
from xdiabetes.channels.matrix import MatrixConfig
...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("xdiabetes.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "xdiabetes.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("xdiabetes.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "xdiabetes.channels.matrix.asyncio.create_task", _fake_create_task
    )
