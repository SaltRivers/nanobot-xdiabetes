...
import xdiabetes.channels.dingtalk as dingtalk_module
from xdiabetes.channels.dingtalk import DingTalkChannel, DingTalkStreamHandler
from xdiabetes.channels.dingtalk import DingTalkConfig
...
async def test_group_send_uses_group_messages_api() -> None:
    ...
    ok = await channel._send_batch_message(
        ...
        "group:conv123",
        "sampleMarkdown",
        {"text": "hello", "title": "X-Diabetes Reply"},
    )
...
async def test_handler_processes_file_message(monkeypatch) -> None:
    ...
    async def fake_download(download_code, filename, sender_id):
        return f"/tmp/xdiabetes_dingtalk/{sender_id}/{filename}"
    ...
    assert "/tmp/xdiabetes_dingtalk/user1/report.xlsx" in msg.content
...
async def test_download_dingtalk_file(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "xdiabetes.config.paths.get_media_dir",
