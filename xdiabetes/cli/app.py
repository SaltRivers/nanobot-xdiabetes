"""Primary CLI entry point for the X-Diabetes runtime."""

import typer

from xdiabetes.cli import commands
...
def main(
    version: bool = typer.Option(
        None, "--version", "-v", callback=commands.version_callback, is_eager=True
    ),
):
...
def gateway(
    ...
    """Start the gateway."""
    commands.gateway(port=port, workspace=workspace, verbose=verbose, config=config)
...
def learning_status(
    ...
    commands.xdiabetes_learning_status(workspace=workspace, config=config, mode=mode)
...
def learning_review(
    ...
    commands.xdiabetes_learning_review(workspace=workspace, config=config, mode=mode)
...
def learning_eval(
    ...
    commands.xdiabetes_learning_eval(draft_id=draft_id, workspace=workspace, config=config, mode=mode)
...
def learning_approve(
    ...
    commands.xdiabetes_learning_approve(draft_id=draft_id, workspace=workspace, config=config, mode=mode)
...
def learning_reject(
    ...
    commands.xdiabetes_learning_reject(
        draft_id=draft_id,
        reason=reason,
        workspace=workspace,
        config=config,
...
def learning_activate(
    ...
    commands.xdiabetes_learning_activate(draft_id=draft_id, workspace=workspace, config=config, mode=mode)
...
def learning_deactivate(
    ...
    commands.xdiabetes_learning_deactivate(
...
def learning_rollback(
    ...
    commands.xdiabetes_learning_rollback(
        skill_name=skill_name,
        workspace=workspace,
        config=config,
        mode=mode,
...
def learning_enable(
    ...
    commands.xdiabetes_learning_enable(config=config)
...
def learning_disable(
    ...
    commands.xdiabetes_learning_disable(config=config)
