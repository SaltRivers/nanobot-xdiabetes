...
# Backward-compatible re-export for existing monkeypatches/tests.
get_workspace_path = _get_workspace_path

app = typer.Typer(
    name="x-diabetes",
    help=f"{__logo__} X-Diabetes - Clinical Workflow CLI",
...
def _init_prompt_session() -> None:
    ...
    from xdiabetes.config.paths import get_cli_history_path
...
def _print_agent_response(response: str, render_markdown: bool) -> None:
    ...
    console.print()
    console.print(f"[cyan]{__logo__} X-Diabetes[/cyan]")
    console.print(body)
    console.print()
...
async def _print_interactive_response(response: str, render_markdown: bool) -> None:
    ...
    def _write() -> None:
        ...
        ansi = _render_interactive_ansi(
            lambda c: (
                c.print(),
                c.print(f"[cyan]{__logo__} X-Diabetes[/cyan]"),
...
class _ThinkingSpinner:
    ...
    def __init__(self, enabled: bool):
        self._spinner = console.status(
            "[dim]X-Diabetes is thinking...[/dim]", spinner="dots"
...
def version_callback(value: bool):
    if value:
        console.print(f"{__logo__} X-Diabetes v{__version__}")
        raise typer.Exit()
...
def _onboard_plugins(config_path: Path) -> None:
    ...
    from xdiabetes.channels.registry import discover_all
...
def _make_provider(config: Config):
    ...
    from xdiabetes.providers.base import GenerationSettings
    ...
    # Custom: direct OpenAI-compatible endpoint, bypasses LiteLLM
        ...
        from xdiabetes.providers.custom_provider import CustomProvider
    ...
    # Azure OpenAI: direct Azure OpenAI endpoint with deployment name
        ...
        if not p or not p.api_key or not p.api_base:
            ...
            console.print("Set them in ~/.x-diabetes/config.json under providers.azure_openai section")
        ...
        from xdiabetes.providers.registry import find_by_name
        spec = find_by_name(provider_name)
        if not model.startswith("bedrock/") and not (p and p.api_key) and not (spec and (spec.is_oauth or spec.is_local)):
            console.print("[red]Error: No API key configured.[/red]")
            console.print("Set one in ~/.x-diabetes/config.json under providers section")
            raise typer.Exit(1)
...
def _load_runtime_config(config: str | None = None, workspace: str | None = None) -> Config:
    ...
    from xdiabetes.config.loader import load_config, set_config_path
...
def _normalize_xdiabetes_runtime_config(
    ...
    *,
    ...
    elif loaded.x_diabetes.workspace != defaults.x_diabetes.workspace:
        active_workspace = loaded.x_diabetes.workspace
    ...
    loaded.x_diabetes.workspace = active_workspace
...
def _load_xdiabetes_runtime_config(
    ...
    """Backward-compatible alias for loading the unified X-Diabetes runtime."""
...
def _prepare_runtime_workspace(config: Config, *, silent: bool = False) -> list[str]:
    ...
    return prepare_xdiabetes_workspace(
        config.workspace_path,
        mode=config.x_diabetes.mode,
        silent=silent,
    )
...
def _print_deprecated_memory_window_notice(config: Config) -> None:
    ...
    if config.agents.defaults.should_warn_deprecated_memory_window:
        console.print(
            ...
            "[cyan]x-diabetes onboard[/cyan] to refresh your config template."
        )
...
def gateway(
    ...
    from xdiabetes.cron.types import CronJob
    from xdiabetes.heartbeat.service import HeartbeatService
    from xdiabetes.session.manager import SessionManager
    ...
    async def on_cron_job(job: CronJob) -> str | None:
        ...
        from xdiabetes.agent.tools.message import MessageTool
        from xdiabetes.utils.evaluator import evaluate_response
...
def xdiabetes_onboard():
    ...
    config.agents.defaults.workspace = config.x_diabetes.workspace
    save_config(config)
...
def xdiabetes_learning_enable(
    ...
    from xdiabetes.config.loader import get_config_path, load_config, save_config, set_config_path
...
def channels_status():
    """Show channel status."""
    from xdiabetes.channels.registry import discover_all
    from xdiabetes.config.loader import load_config
...
def _get_bridge_dir() -> Path:
    ...
    from xdiabetes.config.paths import get_bridge_install_dir
    ...
    pkg_bridge = Path(__file__).parent.parent / "bridge"  # packaged bridge assets
    ...
    if not source:
        ...
        console.print("Try reinstalling: pip install --force-reinstall x-diabetes-ai")
        raise typer.Exit(1)
...
def channels_login():
    ...
    from xdiabetes.config.loader import load_config
    from xdiabetes.config.paths import get_runtime_subdir
...
def plugins_list():
    ...
    from xdiabetes.channels.registry import discover_all, discover_channel_names
    from xdiabetes.config.loader import load_config
...
def status():
    ...
    if config_path.exists():
        from xdiabetes.providers.registry import PROVIDERS
