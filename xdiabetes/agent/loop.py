...
from xdiabetes.agent.memory import MemoryConsolidator
from xdiabetes.agent.subagent import SubagentManager
from xdiabetes.agent.tools.cron import CronTool
from xdiabetes.agent.skills import BUILTIN_SKILLS_DIR
from xdiabetes.agent.tools.filesystem import EditFileTool, ListDirTool, ReadFileTool, WriteFileTool
from xdiabetes.agent.tools.message import MessageTool
from xdiabetes.agent.tools.registry import ToolRegistry
from xdiabetes.agent.tools.shell import ExecTool
from xdiabetes.agent.tools.spawn import SpawnTool
from xdiabetes.agent.tools.web import WebFetchTool, WebSearchTool
from xdiabetes.bus.events import InboundMessage, OutboundMessage
from xdiabetes.bus.queue import MessageBus
from xdiabetes.providers.base import LLMProvider
from xdiabetes.session.manager import Session, SessionManager

if TYPE_CHECKING:
    from xdiabetes.config.schema import ChannelsConfig, ExecToolConfig, WebSearchConfig, XDiabetesConfig
...
class AgentLoop:
    ...
    def __init__(
        ...
        # X-Diabetes is the primary runtime profile. The config object may still
        # carry a legacy `enabled` flag, but tool/runtime registration no longer
        # branches on it.
        self.x_diabetes_config = x_diabetes_config or XDiabetesConfig()
        ...
        if self.x_diabetes_config.learning.enabled:
            try:
    ...
    async def _connect_mcp(self) -> None:
        ...
        from xdiabetes.agent.tools.mcp import connect_mcp_servers
        try:
    ...
    async def _handle_restart(self, msg: InboundMessage) -> None:
        ...
        async def _do_restart():
            ...
            # Use -m x-diabetes instead of sys.argv[0] for Windows compatibility
            # (sys.argv[0] may be just "x-diabetes" without full path on Windows)
            os.execv(sys.executable, [sys.executable, "-m", "xdiabetes"] + sys.argv[1:])
