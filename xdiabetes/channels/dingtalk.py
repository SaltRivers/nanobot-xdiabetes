...
from xdiabetes.config.schema import Base
...
class DingTalkStreamHandler(CallbackHandler):
    ...
    Parses incoming messages and forwards them to the DingTalk runtime channel.
    """
    ...
    async def process(self, message: CallbackMessage):
        ...
        try:
            ...
            # Forward to the runtime via _on_message (non-blocking).
...
class DingTalkChannel(BaseChannel):
    ...
    async def _send_markdown_text(self, token: str, chat_id: str, content: str) -> bool:
        return await self._send_batch_message(
            ...
            chat_id,
            "sampleMarkdown",
            {"text": content, "title": "X-Diabetes Reply"},
        )
    ...
    async def _download_dingtalk_file(
        ...
        from xdiabetes.config.paths import get_media_dir
